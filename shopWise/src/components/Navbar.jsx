...
//   const { wishlistItems, userProfile, setUserProfile } = useContext(AuthContext);
const { wishlistItems, userProfile, logout } = useContext(AuthContext);
  ...
  // Mock Active Drawer Cart Items Layout
  const miniCartItems = [
    {
      id: 1,
  ...
  }, []); // Dependancy array se [showUserDropdown] hata kar clean kiya

  return (
    <header className="w-100 font-sans shadow-sm">
      {/* 1. TOP UTILITY BAR */}
      <div
        className="bg-light border-bottom py-2 px-5 text-secondary"
        style={{ fontSize: "14px" }}
      >
        ...
        <div className="container py-3 px-4">
          <div className="row align-items-center justify-content-between">
            ...
            <div className="col-10 col-md-6 col-lg-5">
              <div
                className="input-group border rounded-2"
            ...
            <div className="col-auto d-flex align-items-center gap-4 text-secondary fs-4 position-relative">
              ...
              <div className="position-relative" ref={userRef}>
                ...
                >
                  {/* 🎯 FIX 1: Direct 'userProfile' state se check kiya avatar image ke liye */}
                  {userProfile ? (
                    <img
                ...
                {showUserDropdown && (
                  ...
                  >
                    {/* 🎯 FIX 2: Direct 'userProfile' check se UI toggle hoga real-time me */}
                    {userProfile ? (
                      <>
                        <h6
                      ...
                      <>
                        ...
                        <div className="d-flex gap-2 mb-3">
                          <Link
                            to="/login"
                            className="btn btn-success btn-sm flex-grow-1 fw-bold text-white border-0"
                            style={{
                          ...
                          <Link
                            to="/register"
                            className="btn btn-light btn-sm flex-grow-1 fw-bold border text-secondary bg-white"
                            style={{ padding: "7px 0", fontSize: "13px" }}
                            onClick={() => setShowUserDropdown(false)}
                          >
                            Register
                          </Link>
                    ...
                    >
                      <Link
                        to="/account"
                        className="text-decoration-none text-secondary hover-success py-1 fw-medium d-flex align-items-center gap-2"
                        onClick={() => setShowUserDropdown(false)}
                      >
                        <i className="bi bi-clock-history text-muted"></i> Order History
                      </Link>
                      <Link
                        to="/favorites" // 🆕 Path sahi kiya favorites page ka
                        className="text-decoration-none text-secondary hover-success py-1 fw-medium d-flex align-items-center gap-2"
                        onClick={() => setShowUserDropdown(false)}
                      >
              ...
              <Link
                to="/account" // 🆕 Favorites route match kiya click transition ke liye
                className="position-relative cursor-pointer text-decoration-none"
              >
                <i
                  className={`bi ${wishlistItems.length > 0 ? "bi-heart-fill text-danger" : "bi-heart text-dark"} fs-5`}
                ></i>
                ...
                {showCartDropdown && (
                  ...
                  >
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      ...
                      <span
                        className="badge rounded-pill bg-light text-secondary border fw-medium px-2 py-1"
                        style={{ fontSize: "11px" }}
                      >
                        {miniCartItems.length} items
                      </span>
                    </div>

                    {/* Mapping List Array Stack items */}
                    <div
                      className="d-flex flex-column gap-2.5 overflow-auto mb-3"
                      style={{ maxHeight: "240px" }}
                    >
                      {miniCartItems.map((item) => (
                        <div
                          key={item.id}
                          className="d-flex gap-2.5 align-items-center border-bottom pb-2.5"
                        >
                          <div
                            className="border rounded bg-light p-1 d-flex align-items-center justify-content-center text-center"
                    ...
                    <div
                      className="d-flex justify-content-between align-items-center border-top pt-3 mb-3 fw-bold text-dark"
                      style={{ fontSize: "15px" }}
                    >
                      <span>Subtotal</span>
                      <span>$302.00</span>
      ...
      <nav className="navbar navbar-expand-lg navbar-light bg-white py-2 px-4 container">
        <div className="container-fluid p-0">
          <div className="collapse navbar-collapse" id="navbarNavDropdown">
            <ul
              className="navbar-nav gap-3 align-items-center fw-medium"
              style={{ color: "#4A5568" }}
            >
              <li className="nav-item">
                <Link className="nav-link text-success active fw-semibold" to="/">
                  Home
                </Link>
              </li>
              <li className="nav-item">
